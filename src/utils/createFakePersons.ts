...
export default function createFakePerson(numberOfPersons: number): Contact[] {
	...
	for (let i = 0; i < numberOfPersons; i++) {
		...
		contactList.push({
			...
			email: fakerPT_BR.internet.email({ firstName, lastName, provider: getProvider() }).toLowerCase(),
